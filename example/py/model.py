...
from utils import weibull_log_pdf, weibull_scale, tensor_slice
...
class GMM_Survival(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.log_c_sigma = tf.exp(tf.Variable(tf.initializers.GlorotNormal()([self.num_clusters, self.encoded_size]), name="sigma"))
    ...
    def call(self, inputs, training=True):
        ...
        z = tfp.distributions.MultivariateNormalDiag(loc=z_mu, scale_diag=tf.math.sqrt(tf.math.exp(log_z_sigma)))
        if training:
        ...
        p_z_c = tf.stack([tf.math.log(
            tfp.distributions.MultivariateNormalDiag(loc=tf.cast(self.c_mu[i, :], tf.float64),
        ...
        #loss_variational_2 = tf.math.reduce_sum(tf.math.xlogy(tf.cast(p_c_z, tf.float64), 1e-60 + tf.cast(p_c_z, tf.float64)), axis=-1)
        ...
        p_z_c = p_z_c[0]    # take the first samplef
        p_c_z = p_c_z[0]
    ...
    def generate_samples(self, j, n_samples):
        z = tfp.distributions.MultivariateNormalDiag(loc=self.c_mu[j, :], scale_diag=tf.math.sqrt(tf.math.exp(self.log_c_sigma[j, :])))
...
class Encoder(layers.Layer):
    def __init__(self, encoded_size):
        ...
        self.mu = tf.keras.layers.Dense(encoded_size, activation=None)
        self.sigma = tf.keras.layers.Dense(encoded_size, activation=None)
...
class Decoder(layers.Layer):
    def __init__(self, input_shape, activation):
        ...
        if activation == "sigmoid":
            self.dense4 = tf.keras.layers.Dense(self.inp_shape, activation="sigmoid")
        else:
            self.dense4 = tf.keras.layers.Dense(self.inp_shape)
